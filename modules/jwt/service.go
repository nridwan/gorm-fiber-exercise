...
type JwtService interface {
	...
	Refresh(claims jwt.MapClaims) (*JWTTokenModel, error)
	GenerateToken(id uuid.UUID, issuer string) (*JWTTokenModel, error)
}
...
func (service *JwtModule) Refresh(claims jwt.MapClaims) (*JWTTokenModel, error) {
	now := time.Now().Unix()
	...
	issuer := claims["sub"].(string)

	// Generate encoded token and send it as response.
	accessToken, err := service.generateAccessToken(sub, issuer, now)
	if err != nil {
		return nil, err
	}
	refreshToken, err := service.generateRefreshToken(sub, issuer, now)
	if err != nil {
		return nil, err
	}
...
func (service *JwtModule) GenerateToken(id uuid.UUID, issuer string) (*JWTTokenModel, error) {
	now := time.Now().Unix()
	// Generate encoded token and send it as response.
	accessToken, err := service.generateAccessToken(id, issuer, now)
	if err != nil {
		return nil, err
	}
	refreshToken, err := service.generateRefreshToken(id, issuer, now)
	if err != nil {
		return nil, err
	}
...
func (service *JwtModule) generateAccessToken(id uuid.UUID, issuer string, now int64) (string, error) {
...
func (service *JwtModule) generateRefreshToken(id uuid.UUID, issuer string, now int64) (string, error) {
	token := jwt.New(jwt.SigningMethodHS256)
